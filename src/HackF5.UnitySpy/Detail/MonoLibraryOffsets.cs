...
namespace HackF5.UnitySpy.Detail
{
    ...
    public class MonoLibraryOffsets
    {
        ...
        public static readonly MonoLibraryOffsets Unity2019_4_5_x64_Offests = new MonoLibraryOffsets
        {
            ...
            TypeDefinitionSize = 0x5c + 0x20 + 0x18 - 0x4,              // 0x90 // Array Element Size
        ...
        public static MonoLibraryOffsets GetOffsets(string unityVersion, bool is64Bits, bool force = true)
        {
            ...
            if (monoLibraryOffsets == null)
            {
                if(force)
                {
                    ...
                    {
                        /*
                        string[] splitedRequestedVersion = unityVersion.Split(".");
                        int requestedVersionYear = splitedRequestedVersion[0];
                        int requestedVersionWithInYear = splitedRequestedVersion[1];
                        int requestedSubversion = splitedRequestedVersion[2];

                        MonoLibraryOffsets monoLibraryOffsets = SupportedVersions[0];
                        string[] unityVersionSplit = monoLibraryOffsets.UnityVersion.Split(".");
                        int requestedVersionYear = splitedRequestedVersion[0];
                        int requestedVersionWithInYear = splitedRequestedVersion[1];
                        int requestedSubversion = splitedRequestedVersion[2];
                        */
